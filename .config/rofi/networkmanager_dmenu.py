...
CONF.read(expanduser(".config/rofi/networkmanager_config.ini"))
...
def create_other_actions(client):
    ...
    actions.append(Action("{} Wifi".format(wifi_action), toggle_wifi,
                      not wifi_enabled))
...
def get_selection(all_actions):
    ...
    if rofi_highlight is True:
        ...
        inp = [(SYM_ACTIVE if action.is_active else '   ') + str(action)
    ...
    if rofi_highlight is False:
        action = [i for i in all_actions
                  if ((str(i).strip() == str(sel.strip())
                      ...
                      (SYM_ACTIVE + str(i) == str(sel.rstrip('\n'))
                       and i.is_active))]
    else:
...
def notify(message, details=None, urgency="low", icon="notification-network-wireless"):
...
def run():
    ...
    actions = combine_actions(ap_actions, eth_actions, vpn_actions, wg_actions,
                              gsm_actions, [], wwan_actions,
